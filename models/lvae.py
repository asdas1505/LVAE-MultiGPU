...
class PHI(nn.Module):
    def __init__(self, n_layers):
        ...
        """
        Parameters of posterior formed by combining Posterior q_{z/x) and prior p_{x/z}
        These parameters are to be updated iteratively
        
        Dont know if ParamterList will work or not
        """
        self.params = nn.ParameterList([nn.Parameter() for i in range(n_layers)])
        self.param_values = [0]*n_layers
...
class LadderVAE(BaseGenerativeModel):
    ...
    def forward(self, x):
        ...
        if self.noise_bool:
            x_degraded = x + self.noise_std * torch.randn_like(x)
            x_degraded = torch.clip(x_degraded, 0, 1)

        # Pad input to make everything easier with conv strides
        x_pad = self.pad_input(x if not self.noise_bool else x_degraded)

        # Bottom-up inference: return list of length n_layers (bottom to top)
        bu_values = self.bottomup_pass(x_pad)

        # Top-down inference/generation
        # for i in range(len(self.top_down_layers)):
    ...
    def forward_evaluate(self, x, eval_iters=300):
        ...
        self.phi = PHI(self.n_layers)
        self.optim_infer = torch.optim.Adam(self.phi.parameters(), lr=1e-3)
        ...
        # Top-down inference/generation
        # Used to initialize the phi
        _, _ = self.topdown_pass(bu_values, eval_bool=True)

        grad_switch(list(self.phi.parameters()), grad_bool=True)

        loss_list = []
        elbo_list = []
        recon_list = []

        for iter in tqdm(range(eval_iters)):
            out, td_data = self.topdown_pass(bu_values, iter_bool=True, eval_bool=True)
            out = crop_img_tensor(out, img_size)
            loss, elbo, recons = self.compute_loss(out, x, td_data)

            self.optim_infer.zero_grad()
            loss.backward()
            self.optim_infer.step()

            loss_list.append(loss)
            elbo_list.append(elbo)
            recon_list.append(recons)

        grad_switch(list(self.phi.parameters()), grad_bool=False)

        out, td_data = self.topdown_pass(bu_values, iter_bool=True, eval_bool=True)
        out = crop_img_tensor(out, img_size)

        elbo_refined, x_rec =  self.compute_loss(out, x, td_data, elbo_bool=True)

        return x_rec, elbo_refined, loss_list, elbo_list, recon_list

    def compute_loss(self, out, x, td_data, elbo_bool=False):
        ll, likelihood_info = self.likelihood(out, x)
        ...
        loss = recons + kl_loss * self.beta

        elbo_sep = -(recons_sep + kl_sep)
        elbo = elbo_sep.mean()

        if elbo_bool:
            return elbo, likelihood_info['sample']

        return loss, elbo, recons
    ...
    def topdown_pass(self,
        ...
        for i in reversed(range(self.n_layers)):
            ...
            if not iter_bool:
                out, out_pre_residual, aux, phi_var = self.top_down_layers[i](
                    out,
                ...
                if eval_bool:
                    self.phi.params[i].data = phi_var
            else:
                out, out_pre_residual, aux, _ = self.top_down_layers[i](
                    out,
                    skip_connection_input=skip_input,
                    inference_mode=inference_mode,
                    bu_value=bu_value,
        ...
        """
        Iterative scheme to be added here
        Refining variable 'out' 
        """
